...
_FAKE_NOISY_WORKER = textwrap.dedent("""\
    from __future__ import annotations
    import argparse, json, sys
    from pathlib import Path
...
def test_kokoro_synthesizer_ignores_non_json_worker_stdout(tmp_path: Path) -> None:
    worker_script = tmp_path / "noisy-worker.py"
    worker_script.write_text(_FAKE_NOISY_WORKER, encoding="utf-8")
    ...
    result = synthesizer.synthesize(SynthesisRequest(text="Chunk rumoroso.", voice="if_sara"))
