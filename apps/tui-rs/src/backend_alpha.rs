use super::{
    decode_payload, AppSnapshot, BackendLogEntry, DocumentListItem, DocumentView,
    RequestEnvelope, ResponseEnvelope, SessionSnapshot,
};
...
impl ProcessBackendClient {
    pub(crate) fn spawn(config_path: Option<&Path>) -> Result<Self, String> {
    ...
    pub(crate) fn get_app_snapshot(&mut self) -> Result<AppSnapshot, String> {
        let response = self.send_request("query", "get_app_snapshot", json!({}))?;
        decode_payload(response.payload, "app")
    }

    pub(crate) fn get_session_snapshot(&mut self) -> Result<Option<SessionSnapshot>, String> {
    ...
    pub(crate) fn get_doctor_report(&mut self) -> Result<Value, String> {
        let response = self.send_request("query", "get_doctor_report", json!({}))?;
        Ok(response.payload)
    }

    pub(crate) fn list_documents(&mut self) -> Result<Vec<DocumentListItem>, String> {
    ...
    pub(crate) fn get_document_view(
        &mut self,
    ...
    pub(crate) fn execute_command_response(
        &mut self,
        name: &str,
        payload: Value,
    ...
    pub(crate) fn recent_stderr_entries(&self, after_sequence: u64) -> Vec<BackendLogEntry> {
